...
public class ViewDragHelper {
    ...
    public static abstract class Callback {
         ...
         * {@link #STATE_SETTLING} and the view capture will not fully end until it comes
         * to a complete stop. If neither of these methods is invoked before
         * <code>onViewReleased</code> returns, the view will stop in place and
         * the ViewDragHelper will return to {@link #STATE_IDLE}.</p>
         *
     ...
     * {@link #processTouchEvent(MotionEvent, View)} receiving an ACTION_CANCEL event.
     */
    void cancel() {
     ...
     *
     * <p>This operation does not count as a capture event, though {@link #mCapturedView}
     ...
     */
    void processTouchEvent(MotionEvent ev, View toCapture) {
        final int action = ev.getActionMasked();
        final int actionIndex = ev.getActionIndex();
...
//                    final View toCapture = findTopChildUnder((int) x, (int) y);
     ...
     */
    private boolean checkTouchSlop(View child, float dy) {
        if (child == null) {
            return false;
        }
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        if (dx != 0) {
            mCapturedView.offsetLeftAndRight(- oldLeft);
        }
        if (dy != 0) {
        ...
        if (dx != 0 || dy != 0) {
            final int clampedDx = left - oldLeft;
            final int clampedDy = clampedY - oldTop;
            mCallback.onViewPositionChanged(mCapturedView, left, clampedY,
                    clampedDx, clampedDy);
        }
    }

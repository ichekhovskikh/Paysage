...
public class CustomBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public interface BottomSheetCallback {
         ...
         */
        default void onStateChanged(@NonNull View bottomSheet, @State int newState) {
        }
         ...
         */
        default void onSlide(@NonNull View bottomSheet, float slideOffset) {
        }
    }
    ...
    private static final int SIGNIFICANT_VEL_THRESHOLD = 50;
    ...
    @State
    private int preScrollState = state;
    ...
    public boolean onInterceptTouchEvent(
        ...
        if (!ignoreEvents
            ...
            // abort and start parent nested scrolling again
            if (settleRunnable != null) {
                nestedScrolled = true;
                settleRunnable.isAborted = true;
            }
            return false;
        }
    ...
    public void setHalfExpandedOffset(int halfExpandedOffset) {
        ...
        if (halfExpandedOffset <= 0) {
            throw new IllegalArgumentException("half expanded offset must be a above 0");
        }
    ...
    public float getHalfExpandedRatio() {
        return (halfExpandedOffset > collapsedOffset || collapsedOffset == getExpandedOffset())
            ? (float) (collapsedOffset - halfExpandedOffset) / (parentHeight - collapsedOffset)
            : (float) (collapsedOffset - halfExpandedOffset) / (collapsedOffset - getExpandedOffset());
    }
    ...
    private void createMaterialShapeDrawable(@NonNull Context context, AttributeSet attrs) {
        this.createMaterialShapeDrawable(context, attrs, false, null);
    }
    ...
    void startSettlingAnimation(View child, int state, int top, boolean settleFromViewDragHelper) {
        ...
        if (startedSettling) {
            ...
            if (!settleRunnable.isPosted) {
                ...
                settleRunnable.isAborted = false;
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (expanded) {
                ...
                if (updateImportantForAccessibilityOnSiblings
                    ...
                    Integer mode = importantForAccessibilityMap.get(child);
                    if (mode != null) {
    ...
    private void setupScrollCallbacks() {
        callbacks.add(new BottomSheetCallback() {
    ...
    private View findVerticalScrollingChild(View view, int direction) {
        if (view instanceof ScrollingView && view.canScrollVertically(direction)) {
            return view;
        }
        if (view instanceof ViewGroup) {
            ViewGroup group = (ViewGroup) view;
            for (int i = 0, count = group.getChildCount(); i < count; i++) {
                View scrollingChild = findVerticalScrollingChild(group.getChildAt(i), direction);
                if (scrollingChild != null) {
                    return scrollingChild;
                }
            }
